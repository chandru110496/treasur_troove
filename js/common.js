$(document).ready(function () {
  ...
  // load all cart function has moved to the cart.php page so bellow has commented -----------<<<<<<<<<<?????????????????

  // function loadAllCart() {
  //   alert('load all cart method');
  //   $(".fetch_cart").html("");
  //   $(".side_bar_cart").html("");
  //   alert('before ajax method');
  //   $.ajax({
  //     url: 'ajax/cart.php',
  //     type: "GET",
  ...
  function loadAllCartCount() {
    ...
    var value_tax = parseFloat($(".product_tax").html());


    // Calculate and set cart total (sum of cart value and tax)
    $(".fetch_cart_total").html(cartValue + cartValueTax + value_tax);
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $.each(data, function (key, value) {
          ...
          console.log(value);
          $total = value.total_price + value.tax + value.value_tax;
  ...
  loadAllCartCountPay();
  $(document).on('click', '.add_to_cart_OLDremoved', function () {
